...
{ 
	...
	</ label="BGM YN", help="Play the Music ?", options="Y,N", order=3 /> bgmYn = "N";
	...
	</ label="시계 보기", help="시계보기(인터넷연결해야 제대로 표시)", options="Yes,No", order=4 /> useClock = "No";
	</ label="컬러 마퀴", help="컬러 마퀴 사용", options="Yes,No", order=5 /> useColorMarquee = "No";
}
...
{
	// Clock
	local dt1 = fe.add_text( "", 0, fly* 0.0087, flw*0.5, flh*0.1000 );
	
	if( my_config["fontColor"] == "WHITE" )
	{
		dt1.set_rgb( 254, 254, 254 );
	}
	else
	{
		dt1.set_rgb( 0, 0, 0 );
	...
	dt1.align = Align.Left;
	dt1.style = Style.Bold;
	dt1.font = "HANNA";
	dt1.char_size = abs(("0"+my_config["fontSize"]).tointeger());
	
	function update_clock( ttime )
	{
		local now = date();		
		dt1.msg = format("%02d", now.hour ) + ":" + format("%02d", now.min );
